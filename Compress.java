...
public class Compress {
  ...
  public static void compress(String fileName) {
    ...
    //groups of 8 bin to CHARS
    String toActuallyWrite = "";
    //System.out.println("Should be " + toWrite.length() / 8 + " values");
    for (int i = 0; i < toWrite.length() / 8; i++) {
      int x = Integer.parseInt(toWrite.substring(i * 8, (i + 1) * 8), 2);
      toActuallyWrite += (char) x;
      //System.out.println(x + " --> " + (char) x);
    }


    FileRW.write(toActuallyWrite, fileName + ".jcompress");
  ...
  public static void decompress(String fileName) {
    ...
    String toWrite = "";
    int count = 0;
    while (fileContents.length() > 1 && count < 3) {
